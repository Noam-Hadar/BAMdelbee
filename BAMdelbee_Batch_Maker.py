...
bams = glob("*.bam") #automatically finds all BAM files within local directory to be used as input files.
print("Located BAM files:\n" +"\n".join(bams) + "\n\nProcessing...")

users = [bam[:-4] for bam in bams] #remove the ".bam" suffix
open("temp/samples.txt", 'w').write('\n'.join(users))
def depthCounter(chromosome):
    try:
        for bam in bams:
            system("samtools depth -a -r chr" + chromosome + ' ' + bam + " | cut -f2,3 | awk '$1%50==0' | head -n -1 | sort -n -s -k1,1 > temp/" + bam[:-4] + "_chr" + chromosome + ".counted") #counts coverage across each chromosome in steps of 50bps
        ...
        for bam in bams[1:]:
            ...
            df = df.merge(df2, on='Coordinate') #merging all count from each bam file to one table
        df['coverages'] = df.apply(lambda x : x.values[1:], axis = 1) 
        df = df[df['coverages'].apply(lambda x : (sum(x) != 0) and (0 in x) and (max(x) >= 5))] #ommits all rows with all zeros or less than 5 reads at max for all samples
        del df['coverages']
