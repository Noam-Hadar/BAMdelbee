...
def generateHeatTable(gene, Chromosome, Position):
    df = pd.read_csv('assets/CleanerVar.tsv', sep = '\t') 
    df = df[df['Gene'] == gene]
    if df.shape[0] == 0:
...
app.layout = html.Div([
    ...
    html.Div(
        dcc.Dropdown(
            id = 'BatchesDropdown',
            options = [{'label': v[7:-4], 'value': v} for v in batches],
            multi = False,
...
def chooseBatch(batch):
    if batch != None:
        ...
        samples = [{'label' : str(sample), 'value' : str(sample)} for sample in sorted(samples, reverse = True)]
        ...
        return [hide, hide, checklist_style, batch[7:-4], [checkList, btn], {'display' : 'none'}, default_style]
    else:
        ...
        return [dropdown_style, ref_style, hide, '', None, default_style, {'display' : 'none'}]
...
def update_figure(clicks, batch, affected, referenceGenome):
    if clicks in (0, None):
        ...
        genes = []
        isCoding = []
        exons = pd.read_csv('assets/' + referenceGenome + '_exons.csv.gz')
        exons['Chromosome'] = exons['Chromosome'].astype(str)
        
        for chromosome in [str(c) for c in range(1,23)] + ['X', 'Y']:
            try:
                df = pd.read_csv(zBatch.open('chr' + chromosome + '.coverage'), sep = '\t')
            except:
                continue
